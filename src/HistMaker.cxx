...
void HistMaker::Loop()
{
   ...
   h_cutflow->GetXaxis()->SetBinLabel(3,"event cleaning");
   ...
   h_cutflow->GetXaxis()->SetBinLabel(8,"n_{fatjet} == 4 && MJ < 600");
   h_cutflow->GetXaxis()->SetBinLabel(9,"n_{fatjet} == 4 && b-tag && MJ < 600");
   h_cutflow->GetXaxis()->SetBinLabel(10,"n_{fatjet} #geq 5 && MJ < 600");
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	if(fatjet_pt->at(i) > m_fatJetPtCut && fabs(fatjet_eta->at(i)) < m_fatJetEtaCut && fatjet_m->at(i)/fatjet_pt->at(i) < m_fatJetMpTCut){
      ...
      iCut++;
      if(nFatJet == 5 && nBTag >=1 && mj < 600){ h_cutflow->Fill(iCut,w); }
   }
   m_outFile->cd();
   m_outFile->Write();
}
